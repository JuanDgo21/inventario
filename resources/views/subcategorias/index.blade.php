<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <a href="{{ route('subcategorias.create') }}" class="inline-block mb-3 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Nueva Subcategoría</a>
        @if(session('success'))
                ...
                <tbody>
                    ...
                    <tr class="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700">
                        ...
                        <td class="px-4 py-2 space-x-2">
                            <a href="{{ route('subcategorias.show', $subcategoria->id) }}" class="inline-block px-3 py-1 bg-blue-500 text-white text-xs rounded hover:bg-blue-600 transition">Ver</a>
                            <a href="{{ route('subcategorias.edit', $subcategoria->id) }}" class="inline-block px-3 py-1 bg-yellow-500 text-white text-xs rounded hover:bg-yellow-600 transition">Editar</a>
                            <form action="{{ route('subcategorias.destroy', $subcategoria->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
